...
def analyze_data_to_stop (data, step_index, flag, time_interval=50, interval_threshold = 100):
    ...
    if time_interval*step_index>18000:
        flag = -1
    ...
    #         flag = False
       ...
       # succeed_seq = data[time_interval * (step_index - 1):time_interval * step_index].values
        #print(np.max(succeed_seq))
       # if np.max(succeed_seq) > 60:
           # flag -= 1
...
def start_reading_data():
    ...
    df_dataset = pd.read_excel(r'C:\Users\17399\Desktop\dataset.xlsx')
    # Assuming that the 'Ending time' column exists and you want to add the filename to the last row
    # If the 'Ending time' column does not exist, you need to create it
    if 'Ending time' not in df_dataset.columns:
        df_dataset['Ending time'] = ''
    # Find the index of the last non-empty row in the 'Ending time' column
    last_non_empty_index = df_dataset['Ending time'].last_valid_index()
    # If there are no non-empty rows in the 'Ending time' column, set the index to -1
    if last_non_empty_index is None:
        last_non_empty_index = -1
    # Add the filename to the 'Ending time' column in the next row
    df_dataset.loc[last_non_empty_index + 1, 'Ending time'] = filename
    # Save the updated DataFrame back to the 'dataset' Excel file
    df_dataset.to_excel(r'C:\Users\17399\Desktop\dataset.xlsx', index=False)
    return data
...
def start_auto_column():
    proportions = ['r1', 'r3', 'r5', 'r7', 'r9', 'r11', 'r13']#
    ...
    send_serial_command(16, '000000', port1)
    send_serial_command(16, '000000', port2)

if __name__ == '__main__':
    port1 = serial.Serial("COM4", 9600, timeout=None)
    port2 = serial.Serial("COM5", 9600, timeout=None)
    port3 = serial.Serial("COM7", 9600, timeout=None)
    port4 = serial.Serial("COM3", 9600, timeout=None)
